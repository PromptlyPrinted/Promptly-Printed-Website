...
async function processProduct(sku: string, country: string, retryCount = 0): Promise<boolean> {
  try {
    ...
    if (error.message?.includes('rate limit') && retryCount < MAX_RETRIES) {
      console.log(`Rate limit hit for SKU ${sku}, waiting ${RATE_LIMIT_DELAY/1000}s before retry...`);
      await delay(RATE_LIMIT_DELAY);
      return processProduct(sku, country, retryCount + 1);
    }
    console.error(`Failed to process SKU ${sku} for country ${country}:`, error);
    return false;
  }
}
...
async function processBatch(skusToProcess: string[], countries: string[]) {
  ...
  for (let i = 0; i < skusToProcess.length; i += BATCH_SIZE) {
    ...
    for (const country of countries) {
      ...
      console.log(`Progress: ${progress}% (${processed} succeeded, ${failed} failed)`);
    }
  }
...
async function clearAndUpdateProducts(skipCleanup = false) {
  ...
  try {
    if (!skipCleanup) {
      ...
      console.log(`Resuming with ${skusToProcess.length} remaining unprocessed SKUs`);
    }

    // Get list of countries to process
    const countries = await getCountries();
    console.log(`Processing ${skusToProcess.length} products for ${countries.length} countries`);
