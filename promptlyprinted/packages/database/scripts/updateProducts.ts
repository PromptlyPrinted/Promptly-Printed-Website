...
function guessProductType(sku: string, description: string): string {
  ...
  if (skuUpper.includes("TEE") || description.toLowerCase().includes("shirt")) return "T-SHIRT";
  if (skuUpper.includes("BAG") || description.toLowerCase().includes("tote")) return "BAG";
  // Fallback
  return "APPAREL";
}
...
async function getProdigiPricing(
  ...
  try {
      ...
      shippingMethods.map(async (method) => {
        ...
        if (!response.ok) {
          if (response.status === 429) {
            console.warn(`Rate limit for SKU ${sku}, waiting 30s before retry...`);
            await delay(30000);
            return null;
          }
          return null;
    ...
    const basePrice = parseFloat(itemsArr[0]?.unitCost?.amount || '0');
...
export async function updateProductData(sku: string) {
  try {
    ...
    if (data.outcome === 'Ok' && data.product) {
      ...
      console.log(`  => Found [${allCountries.size}] countries to process for SKU [${sku}]`);
      ...
      for (const countryCode of allCountries) {
        ...
        console.log(`  => Upserted [${countrySku}] with pricing in [${countryCode}]`);
      }
    } else {
