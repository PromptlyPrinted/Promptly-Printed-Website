...
import {
  type ProductDetails,
  tshirtDetails,
} from './2025_03_26_updateTshirtDetails.js';
...
async function parsePDFSizeChart(
  pdfPath: string
): Promise<Array<{ name: string; measurements: Record<string, string> }>> {
  try {
...
async function fetchAllSizeCharts(): Promise<SizeChartData[]> {
  ...
  for (const [sku, product] of Object.entries(tshirtDetails) as [
    string,
    ProductDetails,
  ][]) {
    try {
...
fetchAllSizeCharts()
  .then(async (sizeCharts) => {
    ...
    console.log(
      'Size charts have been fetched and saved, and T-shirt details have been updated.'
    );
  })
  .catch((error) => {
