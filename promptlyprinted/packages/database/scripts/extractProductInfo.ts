...
async function extractProductInfo(pdfUrl: string): Promise<ProductInfo> {
  try {
    ...
    const sizePattern =
      /\b(XS|S|M|L|XL|2XL|3XL|4XL|5XL|XXL|XXXL|XXXXL|XXXXXL)\b/g;
    const sizes = [...new Set(fullText.match(sizePattern) || [])] as string[];

    // Parse colors - look for color names followed by common descriptors
    const colorPattern =
      /\b(White|Black|Navy|Red|Grey|Gray|Blue|Green|Yellow|Purple|Pink|Brown|Orange|Maroon|Gold|Silver|Natural|Heather[a-zA-Z\s]*)\b/g;
