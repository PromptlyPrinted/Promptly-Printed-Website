...
async function main() {
  for (const [sku, details] of Object.entries(tshirtDetails)) {
    ...
    for (const lang of TARGET_LANGUAGES) {
      ...
      const features = details.features ? await Promise.all(details.features.map(f => translateText(f, lang))) : null;
      const ecoProperties = details.ecoProperties ? await Promise.all(details.ecoProperties.map(f => translateText(f, lang))) : null;
      const careInstructions = details.careInstructions ? await Promise.all(details.careInstructions.map(f => translateText(f, lang))) : null;
      // Upsert translation
      await prisma.productTranslation.upsert({
        where: { productId_languageCode: { productId: product.id, languageCode: lang } },
        update: { name, description, features, ecoProperties, careInstructions },
        create: { productId: product.id, languageCode: lang, name, description, features, ecoProperties, careInstructions },
      });
