...
interface TemporaryImageStore
  extends Map<string, { url: string; timestamp: number; isPublic?: boolean }> {
  IMAGE_TTL: number;
  CLEANUP_INTERVAL: number;
}

export const temporaryImageStore = new Map<
  string,
  { url: string; timestamp: number; isPublic?: boolean }
>() as TemporaryImageStore;
...
temporaryImageStore.set = function (
  key: string,
  value: { url: string; timestamp: number; isPublic?: boolean }
) {
  const entry = {
    ...value,
